...
def hapus_transaksi(id_transaksi: int) -> bool:
    """Menghapus transaksi dari database berdasarkan ID."""
    query = "DELETE FROM transaksi WHERE id = ?"
    try:
        # Menggunakan fungsi execute_query yang sudah ada di database.py kamu
        execute_query(query, (id_transaksi,))
        return True
    except Exception as e:
        print(f"Error di database.py saat menghapus data: {e}")
        return False